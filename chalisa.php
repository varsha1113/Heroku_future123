...
if (isset($_POST['submit'])) {
    ...
    $query=mysqli_query($con, "insert into Chaleesa(title,content,image)values('$title','$content','$image_name')");
    if ($query) {
        echo '<script type="text/javascript">alert(" FILE UPLOADED ");</script>';
        mysqli_close($con); // Close connection
        header("location:admin-3.php"); // redirects to all records page
        exit;
    }else{
